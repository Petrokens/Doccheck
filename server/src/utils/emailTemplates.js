          ...
          <table role="presentation" width="560" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
            <tr>
              <td style="background:#0f3d3e;color:#ffffff;padding:20px 28px;">
                <div style="font-size:11px;letter-spacing:0.16em;text-transform:uppercase;opacity:0.8;">DocCheck AI</div>
            ...
            <tr>
              <td style="padding:16px 28px 24px;color:#6b7280;font-size:12px;border-top:1px solid #e5e7eb;">
                This message was sent by DocCheck AI. If you were not expecting it, ignore this email.
              </td>
            </tr>
          </table>
        </td>
...
function credentialsEmail({ username, email, password, loginUrl }) {
  const heading = 'Your DocCheck AI login credentials';
  const bodyHtml = `
    <p style="margin:0 0 12px;">Hi ${escapeHtml(username || 'there')},</p>
    <p style="margin:0 0 16px;">A Master Admin created your DocCheck AI account. Use these details to sign in:</p>
...
function loginOtpEmail({ username, otp }) {
  ...
  const bodyHtml = `
    <p style="margin:0 0 12px;">Hi ${escapeHtml(username || 'there')},</p>
    <p style="margin:0 0 16px;">Use this one-time code to finish signing in to DocCheck AI. It expires in 10 minutes.</p>
  ...
  return {
    ...
    html: layout({ heading, bodyHtml }),
    text: `Hi ${username || 'there'},\nYour DocCheck AI login code is ${otp}. It expires in 10 minutes.`,
  };
}
...
function passwordResetEmail({ resetUrl }) {
  ...
  const bodyHtml = `
    <p style="margin:0 0 16px;">Use the button below to choose a new DocCheck AI password. This link expires in 30 minutes.</p>
